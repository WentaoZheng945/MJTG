...
def trajectory_plot_ego_and_adversary(case_scene, scenario_id):
    ...
    for i in range(4):
        ...
        if i == 0:
            ...
            label = 'adversary' + '_Lon'
        else:
            label = 'adversary' + '_Lat'
        title = str(scenario_id) + '_' + str(label)

        # 第一张子图:累计位移
        plt.subplot(3, 1, 1)  # 累计位移
        plt.plot(time, case_scene.fusion_xy[scenario_id, :, i], '-*k', linewidth=0.25, markersize=1.5, label=label+'_Org')  # 实线 * 黑色 *大小1.5 线宽2.5
        plt.plot(time, case_scene.fusion_xy_repair[scenario_id, :, i], '-*g', linewidth=0.25, markersize=0.5, label=label+str('_Enhd'))
        ...
        # 第二张子图:速度
        plt.subplot(3, 1, 2)
        plt.plot(time, case_scene.fusion_v[scenario_id, :, i], '-*k', linewidth=0.25, markersize=1.5, label=label+'_Org')  # 实线 * 黑色 *大小1.5 线宽2.5
        plt.plot(time, case_scene.fusion_v_repair[scenario_id, :, i], '-*g', linewidth=0.25, markersize=0.5, label=label + str('_Enhd'))
        plt.legend(prop={'size': 10})
        ...
        # 第三张子图:加速度
        plt.subplot(3, 1, 3)
        plt.plot(time, case_scene.fusion_a[scenario_id, :, i], '-*k', linewidth=0.25, markersize=1.5, label=label + str('_Org'))  # 实线 * 黑色 *大小1.5 线宽2.5
        plt.plot(time, case_scene.fusion_a_repair[scenario_id, :, i], '-*g', linewidth=0.25, markersize=0.5, label=label + str('_Enhd'))
        plt.legend(prop={'size': 10})
...
if __name__ == '__main__':
    ...
    global_sampleScene_without_latent_path = r'./processed_by_zwt/samples/data_without_latent_cons.npy'
    ...
    global_reconstruction_path = r'./processed_by_zwt/test/data_for_valid.npy'
