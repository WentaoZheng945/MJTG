...
class Repair_cutin():
    def __init__(self, frame_rate=0.04, lon_bound=[-8, 5], lat_bound=[-3.9, 3.9], fusion_path='', isNorm=True, isXy=True, lims=[], need_repair=True) -> None:
    ...
    def sceneRecoverPosition(self):
        ...
        for i in range(self.fusion.shape[0]):  # 遍历每一个场景
    ...
    def sceneEnhace(self):
        ...
        if not self.need_repair:
            ...
            print("跳过数据强化环节")
            return
        np.save('./processed_by_zwt/output/cutin3_re.npy', self.fusion_xy)
        ...
        for scene_ind in tqdm(range(self.fusion_xy.shape[0])):
            ...
            for i in range(2):
                ...
                if df_ego.loc[0, 'global_center_x'] > df_ego.loc[len(df_ego) - 1, 'global_center_x']:
                    ...
                    df_ego.loc[:, 'global_center_y'] = df_ego.loc[:, 'global_center_y'] * -1  # y反转的意义？
                ...
                if not out_trj.empty:
                    ...
                    if i == 0:
                        ...
                        if self.fusion_xy_repair.size > 0:
                            ...
                            self.fusion_v_repair = temp_array_v.reshape([1, -1, 6])  # 对张量(6, 125)转置为(125, 6)，再reshape为(1, 125, 6)
...
if __name__ == '__main__':
    ...
    if not isSample:
        # 针对重建效果的评价
        inputScene = Repair_cutin(fusion_path='./VAE_for_GAN/argoverse-data/cutin3_xy_singleD_proceseed_norm_test.npy', lims=lims, need_repair=False)
        reconScene = Repair_cutin(fusion_path='./VAE_for_GAN/test/data_for_vaild.npy', lims=lims, need_repair=False)
        eval = reconScene.reconEval(inputScene.fusion_xy)
        print('ADE: {}, FDE: {}'.format(str(eval[0]), str(eval[1])))
        print(reconScene.isCutin())
        reconScene.isReal()
    else:
        # originalScene = Repair_cutin(fusion_path='./VAE_for_GAN/samples/data_without_latent_cons.npy', lims=lims, need_repair=False)
        # originalScene.isReal()
        sampleScene = Repair_cutin(fusion_path='./VAE_for_GAN/samples/data_without_latent_cons.npy', lims=lims, need_repair=False)
        sampleScene.isReal()
    ...
    sampleScene_orig = Repair_cutin(fusion_path='./VAE_for_GAN/output/cutin3.npy', isNorm=False, need_repair=False)
    # sampleScene_processed = Repair_cutin(fusion_path='./VAE_for_GAN/output/cutin3_processed.npy', isNorm=False, need_repair=False)
    ...
    for i in range(sampleScene_orig.fusion_v_repair.shape[0]):
        ...
        for j in range(2):
            plt.subplot(3, 1, 1)
            plt.plot(time, sampleScene_orig.fusion_xy[i, :, j + 2], '--k', linewidth=0.25, label='Original')
            # plt.plot(time, sampleScene_processed.fusion_xy_repair[i, :, j], '-*g', linewidth=0.25, label='Processed', markersize=0.5)
            plt.ylabel('Position (m)')
            plt.legend(prop={'size': 6})
            trj_title = 'Scenario ' + str(i) + ' Challenger' + ' Direction' + correctness_dir[j] + ' Sample from VAE'
            plt.title(trj_title)
            plt.subplot(3, 1, 2)
            plt.plot(time, sampleScene_orig.fusion_v[i, :, j + 2], '--k', linewidth=0.25, label='Original')
            ...
            if not j:
                ...
                plt.ylim([-10, 10])
            plt.subplot(3, 1, 3)
            plt.plot(time, sampleScene_orig.fusion_a[i, :, j + 2], '--k', linewidth=0.25, label='Original')
            ...
            if not j:
                ...
                plt.ylim([-5, 5])
            if not os.path.exists('./VAE_for_GAN/image/trajectory_process_cutin14'):
                os.makedirs('./VAE_for_GAN/image/trajectory_process_cutin14')
            trj_save_title = './VAE_for_GAN/image/trajectory_process_cutin14/' + trj_title + '.png'
