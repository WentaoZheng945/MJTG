...
class Repair_cutin():
    def __init__(self, frame_rate=0.04, lon_bound=[-8, 5], lat_bound=[-3.5, 3.5], fusion_path='', isNorm=True, isXy=True, lims=[]) -> None:
        ...
        场景数据读入后将自动执行：1.归一化数据的还原；2. 反推主车、对手车的速度、加速度、加加速度；3. 数据增强（离群点剔除、滤波）；4. 计算各场景危险程度Min_Mttc
        ...
        self.fusion = np.load(fusion_path)  # 加载数据（坐标型）
        self.fusion_re = np.array([])  # 用来储存还原后的坐标
        self.fusion_xy = np.array([])  # fusion_re的深copy
        self.fusion_v = np.array([])  # 基于还原后位置坐标差分得到
        self.fusion_a = np.array([])  # 基于还原后位置（速度）坐标差分得到
        self.fusion_j = np.array([])  # 基于还原后位置（加速度）坐标差分得到
        if not self.isXy:
            self.sceneRecoverVelocity()
        else:
            self.sceneRecoverPosition()

        self.fusion_v_outliar = np.array([])  # 去除离群点之后的速度
        self.fusion_v_denosie = np.array([])  # 去噪之后的速度
        self.fusion_xy_repair = np.array([])  # 数据增强之后的坐标
    ...
    def sceneRecoverVelocity(self):
        '''将传入的场景参数还原
        1. 将归一化的逻辑参数fusion，还原为原始量纲fusion_re
        2. 将逻辑参数fusion_re，还原为xy坐标序列fusion_xy
        '''
        print(self.fusion.shape[0])
        for i in range(self.fusion.shape[0]):
            delta_x = self.fusion[i, :, 0].tolist()
            delta_y = self.fusion[i, :, 1].tolist()
            vx_cutin = self.fusion[i, :, 2].tolist()
            vy_cutin = self.fusion[i, :, 3].tolist()

            # TODO: 将归一化的数据还原，并储存在fusion_re中
            if self.isNorm:
                delta_x = restore(delta_x, self.lims[0], self.lims[1])
                delta_y = restore(delta_y, self.lims[2], self.lims[3])
                vx_cutin = restore(vx_cutin, self.lims[4], self.lims[5])
                vy_cutin = restore(vy_cutin, self.lims[6], self.lims[7])
            temp_array1 = np.empty((len(delta_x), 4), dtype=np.float64)
            temp_array1[:, 0] = delta_x
            temp_array1[:, 1] = delta_y
            temp_array1[:, 2] = vx_cutin
            temp_array1[:, 3] = vy_cutin

            if self.fusion_re.size > 0:
                self.fusion_re = np.concatenate((self.fusion_re, temp_array1.reshape([1, -1, 4])), axis=0)
            else:
                self.fusion_re = temp_array1.reshape([1, -1, 4])

            # TODO: 将完成还原的逻辑参数序列反推回xy坐标序列
            x1, x2 = [0], [-delta_x[0]]
            y1, y2 = [0], [-delta_y[0]]
            for i in range(1, len(delta_x)):
                x2.append(x2[i - 1] + vx_cutin[i - 1] * self.rate)
                y2.append(y2[i - 1] + vy_cutin[i - 1] * self.rate)
            x1 = [0] + [delta_x[i] + x2[i] for i in range(1, len(x2))]
            y1 = [0] + [delta_y[i] + y2[i] for i in range(1, len(x2))]
            temp_array = np.empty((len(x1), 4), dtype=np.float64)
            temp_array[:, 0] = x1
            temp_array[:, 1] = y1
            temp_array[:, 2] = x2
            temp_array[:, 3] = y2
            if self.fusion_xy.size > 0:
                self.fusion_xy = np.concatenate((self.fusion_xy, temp_array.reshape([1, -1, 4])), axis=0)
            else:
                self.fusion_xy = temp_array.reshape([1, -1, 4])

            # TODO: 获得场景中车辆的速度信息[ego_x, ego_y, cutin_x, cutin_y], shape(4, 125)
            velocity, accer, jerk = get_speed_and_accer(temp_array, self.rate)
            if self.fusion_v.size > 0:
                self.fusion_v = np.concatenate((self.fusion_v, velocity.transpose().reshape([1, -1, 4])), axis=0)
                self.fusion_a = np.concatenate((self.fusion_a, accer.transpose().reshape([1, -1, 4])), axis=0)
                self.fusion_j = np.concatenate((self.fusion_j, jerk.transpose().reshape([1, -1, 4])), axis=0)
            else:
                self.fusion_v = velocity.transpose().reshape([1, -1, 4])  # 对张量(4, 125)转置为(125, 4)，再reshape为(1, 125, 4)
                self.fusion_a = accer.transpose().reshape([1, -1, 4])
                self.fusion_j = jerk.transpose().reshape([1, -1, 4])
        return
    ...
    def sceneRecoverPosition(self):
        ...
        '''
        print(self.fusion.shape[0])
        for i in range(self.fusion.shape[0]):  # 逐个场景
            ...
            # TODO: 将归一化的数据还原，并储存在fusion_re中
            if self.isNorm:  # 从(0,1)还原成坐标系中坐标
            ...
            temp_array1[:, 4] = x3
            temp_array1[:, 5] = y3
            # 把每个场景的轨迹[125, 6]，拼在一起[batch_size, 125, 6]
            if self.fusion_re.size > 0:
            ...
            # 将每个场景的速度、加速度、jerk[6, 125], 拼接起来[batch_size, 125, 6]
            velocity, accer, jerk = get_speed_and_accer(temp_array1, self.rate)  # 对应单个场景[ego_x, ego_y, cutin_x, cutin_y, pre_x, pre_y], shape(6, 125)
            if self.fusion_v.size > 0:
    ...
    def sceneEnhace(self):
        ...
        """
        # TODO: jerk值分析
        ego_x_jerk = jerkAnalysis(self.fusion_j[:, :, 0])  # 计算每个方向的jerk极端突变值比例
        ...
        for scene_ind in range(self.fusion_xy.shape[0]):
            ego_x = self.fusion_xy[scene_ind, :, 0]
            ego_y = self.fusion_xy[scene_ind, :, 1]
            cutin_x = self.fusion_xy[scene_ind, :, 2]
            cutin_y = self.fusion_xy[scene_ind, :, 3]
            ego_vx = self.fusion_v[scene_ind, :, 0]
            ego_vy = self.fusion_v[scene_ind, :, 1]
            cutin_vx = self.fusion_v[scene_ind, :, 2]
            cutin_vy = self.fusion_v[scene_ind, :, 3]
            ...
            if not out_trj.empty:
                ...
                temp_array_xy = np.dstack((ego_x_repair[0], ego_y_repair[0], cutin_x_repair[0], cutin_y_repair[0]))
                temp_array_v = np.dstack((ego_vx_repair[0], ego_vy_repair[0], cutin_vx_repair[0], cutin_vy_repair[0]))
                temp_array_a = np.dstack((ego_ax_repair[0], ego_ay_repair[0], cutin_ax_repair[0], cutin_ay_repair[0]))
                if self.fusion_xy_repair.size > 0:
                    self.fusion_xy_repair = np.concatenate((self.fusion_xy_repair, temp_array_xy.reshape([1, -1, 4])), axis=0)
                    self.fusion_v_repair = np.concatenate((self.fusion_v_repair, temp_array_v.reshape([1, -1, 4])), axis=0)
                    self.fusion_a_repair = np.concatenate((self.fusion_a_repair, temp_array_a.reshape([1, -1, 4])), axis=0)
                else:
                    self.fusion_xy_repair = temp_array_xy.reshape([1, -1, 4])
                    self.fusion_v_repair = temp_array_v.reshape([1, -1, 4])  # 对张量(4, 125)转置为(125, 4)，再reshape为(1, 125, 4)
                    self.fusion_a_repair = temp_array_a.reshape([1, -1, 4])
            else:
                noSolution_count += 1
        '''
        '''
    ...
    def isVaild(self, test, test_xy):
        ...
        def getPattern(nums):
            '''将序列转换为模式序列'''
            nums = nums[:, ::3]  # [batch_size, 41]
            diff_num = np.diff(nums, axis=1)  # [batch_size, 40]
        ...
        pattern_ego = getPattern(v_ego)  # v_ego: [batch_size, len_seq] pattern_ego: [batch_size, 40]
        true_v_ego = np.array([])
        for i in test_xy[:, :, 0]:
            temp_v = np.diff(i, axis=0)  # 计算每帧下的速度
            temp_v = (temp_v / self.rate)
        ...
        pattern_true_ego = getPattern(true_v_ego)  # pattern_true_ego: [batch_size, 40]

        # 可视化验证
        denoise = wavefilter(self.fusion_re[:, :, 0])
        for i in range(v_ego.shape[0]):
            plt.rcParams['font.sans-serif'] = ['SimHei']  # 显示中文
            plt.ion()
            plt.subplot(2, 1, 1)
            plt.title('真值（上）及泛化值（下）曲线')
            plt.plot(test[i, :, 0])  # 默认横坐标为1,2,3,4
            plt.ylim(-200, 0)
            plt.subplot(2, 1, 2)
            plt.plot(denoise[i])
            plt.ylim(-200, 0)
        ...
        ego = []
        for i in range(v_ego.shape[0]):
            r, p = stats.pearsonr(pattern_true_ego[i, :], pattern_ego[i, :])  # r相关性，p值显著性水平
            ego.append([r, p])
        # 对手车横纵向速度相关性
        pattern_dis_cutin = []  # 对手车横纵向速度的模式距离 TODO 横纵向都算了？
        pearson_cutin = []
        for i in range(2, 4):
    ...
    def getMttc(self):
        ...
        for i in range(self.fusion_xy_repair.shape[0]):
            if not self.realFlag[i]:  # 只计算满足功能性检查的场景
                continue
            cur_mttc = []
            cur_thw = []
            for j in range(self.fusion_xy_repair.shape[1] - 1, -1, -1):
                if abs(self.fusion_xy_repair[i, j, 1] - self.fusion_xy_repair[i, j, 3]) <= 1.5:  # 检查主车和对手车的横向距离是否小于1.5
                    dist = abs(self.fusion_xy_repair[i, j, 0] - self.fusion_xy_repair[i, j, 2])  # 计算纵向距离
                    delta_v = abs(self.fusion_v_repair[i, j, 0]) - abs(self.fusion_v_repair[i, j, 2])  # 速度差
                ...
                if abs(self.fusion_xy_repair[i, j, 1] - self.fusion_xy_repair[i, j, 3]) <= 1.5:
                    ...
                    if delta_a != 0:
                        ...
                        if t1 > 0 and t2 > 0:
                            ...
                            cur_mttc.append(max(t1, t2))
                    cur_thw.append(dist / self.fusion_v_repair[i, j, 0])
                else:
                    cur_mttc.append(float('inf'))
                    cur_thw.append(float('inf'))
                    break
...
if __name__ == '__main__':
    ...
    if not isSample:
        # 针对重建效果的评价
        inputScene = Repair_cutin(fusion_path='./VAE_for_GAN/argoverse-data/cutin3_xy_singleD_proceseed_norm_test.npy', lims=lims)
        ...
        sampleScene = Repair_cutin(fusion_path='./VAE_for_GAN/samples/data_without_latent_cons.npy', lims=lims)
        print(sampleScene.isCutin())
        sampleScene.isReal()

    '''
    # 针对泛化场景多样性的评价
    inputScene = Repair_cutin(fusion_path='./processed_by_zwt/input_data/cutin3_xy_singleD_proceseed_norm_train.npy', lims=lims)
    sampleScene = Repair_cutin(fusion_path='./processed_by_zwt/samples/data_without_latent_cons.npy', lims=lims)
    testScene = Repair_cutin(fusion_path='./processed_by_zwt/input_data/cutin3_xy_singleD_proceseed_norm_test.npy', lims=lims)
    reconstruction = Repair_cutin(fusion_path='./processed_by_zwt/test/data_for_vaild.npy', lims=lims)
    ...
    ax1.set_ylabel('MTTC')
    ax1.set_xlim(-5, 80)
    plt.savefig('./processed_by_zwt/image/minMTTC.png', dpi=600)
    # 绘制THW分布
    plt.clf()
    ax2 = plt.subplot(111)
    ...
    plt.savefig('./processed_by_zwt/image/minTHW.png', dpi=600)
    plt.tight_layout()
