...
def restore(pos, min_p, max_p):
    ...
    """
    pos = [min_p + i * (max_p - min_p) for i in pos]  # x' = (x-xmin)/(xmax-xmin)
    return pos
...
def get_speed_and_accer(position_series, rate):
    ...
    velocity = np.array([[0] * len(x1) for _ in range(6)]).astype(float)  # shape(6, len(x1))
    ...
    for i in range(0, len(x1)):
        if i == 0:
            for j in range(6):
                velocity[j, i] = (positions[j, i + 2] - positions[j, i]) / float(2 * rate)  # 第一帧
        elif i == len(x1) - 1:
            for j in range(6):
                velocity[j, i] = (positions[j, i] - positions[j, i - 2]) / float(2 * rate)  # 最后一帧
        else:
            for j in range(6):
                velocity[j, i] = (positions[j, i + 1] - positions[j, i - 1]) / float(2 * rate)  # 中间帧

    # 差分计算加速度（由前后两个点差分得到）
    for i in range(0, len(x1)):
        if i == 0:
            for j in range(6):
                accer[j, i] = (velocity[j, i + 2] - velocity[j, i]) / float(2 * rate)  # 第一帧
        elif i == len(x1) - 1:
            for j in range(6):
                accer[j, i] = (velocity[j, i] - velocity[j, i - 2]) / float(2 * rate)  # 最后一帧
        else:
            for j in range(6):
                accer[j, i] = (velocity[j, i + 1] - velocity[j, i - 1]) / float(2 * rate)  # 中间帧
    # 差分计算加加速度（中心差分）
    for i in range(0, len(x1)):
        if i == 0:
            for j in range(6):
                jerk[j, i] = (accer[j, i + 2] - accer[j, i]) / float(2 * rate)  # 第一帧
        elif i == len(x1) - 1:
            for j in range(6):
                jerk[j, i] = (accer[j, i] - accer[j, i - 2]) / float(2 * rate)  # 最后一帧
        else:
            for j in range(6):
                jerk[j, i] = (accer[j, i + 1] - accer[j, i - 1]) / float(2 * rate)  # 中间帧
...
def wavefilter(input):
    """小波滤波——多贝西6，高频全置零
    具体参考：https://blog.csdn.net/JingpengSun/article/details/128997306
    ...
    for i in range(input.shape[0]):
        ...
        max_level = pywt.dwt_max_level(datalength, wavelet.dec_len)  # 根据数据长度计算分解层数
        ...
        # pywt.wavedec的三个输入参数分别为：输入信号，小波基，层数
        coeffs = pywt.wavedec(data, wname, mode='smooth', level=max_level)  # 将信号进行小波分解
        # thresholding
...
def outliarRemoval(a, v, a_bound, flag=True):
    ...
    for i in repair_ind:
        if i[1] - 1 >= 0 and i[1] + 1 < a.shape[1]:  # 这里漏了i[1]=124的情况
            if flag:
                ...
                v[i[0], i[1]] = v_new  # 这个的去除离群点仅仅是用周围两点做平均，没有使用基于时间窗口的优化方法
            repair_table[i[0]] += 1
