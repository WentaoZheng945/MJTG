...
# developed time: 2023/7/21 17:38
import os.path
...
class DangerFilter():
    def __init__(self, lims=None, frame_rate=0.04, isNorm=True, fusion_path='', save_path='./processed_by_zwt/input_data', save=True):
    ...
    def split_train_dataset(self):
        ...
        np.save(os.path.join(self.save_path, 'cutin3_xy_singleD_proceseed_norm_train_danger_3.npy'), danger_npy)
        np.save(os.path.join(self.save_path, 'cutin3_xy_singleD_proceseed_norm_train_safe_3.npy'), safe_npy)
        print('[*] 划分数据集并成功保存')
        return
